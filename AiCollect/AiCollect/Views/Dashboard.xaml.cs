...
using Configuration = AiCollect.Models.Configuration;
    ...
    public partial class Dashboard : ContentPage, PromptPageState, QuestionairePageState, ConfigurationPageState, LocationPageState
    {
        ...
        private async void OnQuestionaireTapped(object sender, ItemTappedEventArgs args)
        {
            try
            {
                ...
                if(questionaire.Categories.Count > 0 || questionaire.Regions.Count > 0)
                    ...
                    await Navigation.PushAsync(new QSections(ObjectType.None, questionaire, this));
            }
            catch (Exception ex)
            {
        ...
        private void Init()
        {
            try
            {
                ...
                var qns = configuration.Questionaires.Where(x => x.Status == Statuses.Partial || x.Status == Statuses.Completed).ToList();
        ...
        private async void StartQuestionaire(object sender, EventArgs args)
        {
            try
            {
                ...
                if (configuration.Questionaires.Where(x => x.Status == Statuses.Template).ToList().Count > 1)
                {
                    PromptQuestionaireSelection();
                }
                else if (configuration.Questionaires.Where(x => x.Status == Statuses.Template).ToList().Count == 1)
                {
        ...
        private void OnRefreshQuestionaires(object sender, EventArgs args)
        {
            try
            {
                _template = configuration.Questionaires.Find(q => q.Status == Statuses.Template);
        ...
        void RefreshQuestionaires(Questionaire questionaire)
        {
            try
            {
                configuration.Questionaires.Where(x => x.Status == Statuses.Completed || x.Status == Statuses.Partial)
                                           .ForEach(q =>
                                           {
